...
echo "cmsRun -e -j FrameworkJobReport.xml sim_step.py "$3" inputName=genStep.root outputName=simStep.root"
cmsRun -e -j FrameworkJobReport.xml sim_step.py $3 inputName=genStep.root outputName=simStep.root
rm genStep*.root
...
echo "cmsRun -e -j FrameworkJobReport.xml miniaod_step.py "$3" inputName=recoStep.root outputName=miniaodStep.root" 
cmsRun -e -j FrameworkJobReport.xml miniaod_step.py $3 inputName=recoStep.root outputName=miniaodStep.root"
rm recoStep.root

echo "cmsRun -e -j FrameworkJobReport.xml nanoaod_step.py "$3" inputName=miniaodStep.root "$4
cmsRun -e -j FrameworkJobReport.xml nanoaod_step.py $3 inputName=miniaodStep.root $4
rm miniaodStep.root
